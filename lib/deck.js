...
const humanShuffle = (cards) => {
	...
	while (!_.isEmpty(_.concat(left, right))) {
		cards = _.isEmpty(left) ? cards : cards.concat(left.splice(0, count123Weighted()));
		cards = _.isEmpty(right) ? cards : cards.concat(right.splice(0, count123Weighted()));
	}
	return cards;
};
...
class Deck extends Cards {
	...
	deal() {
		...
		return {
			h1: new Pile(_.concat(p1a, p1b)),
			h2: new Pile(_.concat(p2a, p2b)),
			h3: new Pile(_.concat(p3a, p3b)),
			t: new Pile(t)
		};
	}
}
