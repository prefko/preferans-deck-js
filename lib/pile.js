...
class Pile extends Cards {
	...
	sort(config) {
		...
		if (suits === 4) {
			if (config.sorting === SORTING.RED) this.cards = _.concat(diamond, spade, heart, club);
			if (config.sorting === SORTING.BLACK) this.cards = _.concat(spade, diamond, club, heart);
			return this;
		}
