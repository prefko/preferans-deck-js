...
import PrefDeckTrick from '../src/pref.deck.Trick';
...
describe('PrefDeckTrick tests', () => {
	...
	describe('PrefDeckTrick throw 2 cards test', () => {
		...
		it('PrefDeckTrick throw 2 cards test', () => {
			...
			expect(trick.string).to.be.equal(
				'{' + '"players":2,' + '"first":{"card":"7Club","player":"p1"},' + '"second":{"card":"KSpade","player":"p2"},' + '"third":{},' + '"trump":"",' + '"winner":"p1"' + '}'
			);
		});
	});
	...
	describe('PrefDeckTrick throw 2 cards test with trump', () => {
		...
		it('PrefDeckTrick throw 2 cards test', () => {
			...
			expect(trick.ppn).to.be.equal('P7');
			expect(trick.string).to.be.equal(
				'{"players":2,' + '"first":{"card":"7Club","player":"p1"},' + '"second":{"card":"KSpade","player":"p2"},' + '"third":{},' + '"trump":"Heart",' + '"winner":"p1"' + '}'
			);
		});
	});
	...
	describe('PrefDeckTrick throw 2 cards test with trump', () => {
		...
		it('PrefDeckTrick throw 2 cards test', () => {
			...
			expect(trick.string).to.be.equal(
				'{"players":2,' + '"first":{"card":"7Club","player":"p1"},' + '"second":{"card":"7Heart","player":"p2"},' + '"third":{},' + '"trump":"Heart",' + '"winner":"p2"' + '}'
			);
		});
	});
